...
int main() {
 ...
 while (true) {
  ...
  if (l == "s") {
   ...
   std::getline(std::cin, o);
   if (o == "s") {
    if (!std::filesystem::exists("license.txt")) {
     fn = "license.txt";
    } else {
     while (std::filesystem::exists("license" + std::to_string(n) + ".txt")) {
      n++;
     }
     fn = "license" + std::to_string(n) + ".txt";
    }
    std::ofstream f(fn);
    if (f.is_open()) {
     f << copr;
